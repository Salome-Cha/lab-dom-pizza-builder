...
function renderWhiteSauce() {
  ...
  sauce.forEach (element =>
    { if (state.whiteSauce) {
      sauce.removeAttribute(".white-sauce")
    } else {
      sauce.setAttribute(".white white-sauce")
    }
  });
}
...
function renderPrice() {
  ...
  let price = 10;
  let parent = document.getElementById("parent-ul");
  let newLi = document.createElement("li");
  parent.appendChild(newLi)

  if (state.pepperoni) {
    newLi.innerHTML = "$1 pepperonni";
    price += 1};
  if (state.mushrooms) {
    newLi.innerHTML = "$1 mushrooms";
    price += 1};
  if (state.greenPeppers) {
    newLi.innerHTML = "greenPeppers";
    price += 1};
  if (state.whiteSauce) {
    newLi.innerHTML = "whiteSauce";
    price += 3};
  if (state.glutenFreeCrust) {
    newLi.innerHTML = "glutenFreeCrust";
    price += 5};
...
document.querySelector('btn.btn-crust.active').addEventListener('click', () => {
